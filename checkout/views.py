...
def delivery_option(request):
    if request.method == 'POST':
        ...
        if delivery_option_form.is_valid():
            cleaned_data = delivery_option_form.cleaned_data
            delivery_option = cleaned_data['delivery_option']
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            for item_id, quantity in basket.items():
                try:
                    ...
                    messages.error(
                        request, (
                        "One of the menu items in your basket wasn't found"
                        "In our database. "
            ...
            messages.error(
                ...
                "can you  Please double check your information.")
            return redirect(reverse('checkout'))
        ...
        if (delivery_option == 'delivery' and
                total < DELIVERY_MINIMUM_ORDER_SPEND):
            messages.error(
                request,
                f"Your order must be at least"
                f"€{DELIVERY_MINIMUM_ORDER_SPEND} for delivery."
                )
            return redirect(reverse('delivery_option'))
